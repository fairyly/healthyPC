  ...
  <div class="page-container">
    ...
    <div class="form-card">
      <div class="form-row">
        ...
        <div class="form-group">
          ...
          <input v-model.number="form.systolic" type="number" placeholder="高压" min="0" />
        </div>
        <div class="form-group">
          <label>舒张压 (mmHg)</label>
          <input v-model.number="form.diastolic" type="number" placeholder="低压" min="0" />
        </div>
        <div class="form-group">
          <label>测量时间</label>
    ...
    <div class="record-list">
      ...
      <table v-if="records.length" class="record-table">
        <thead>
          <tr><th>姓名</th><th>收缩压</th><th>舒张压</th><th>测量时间</th><th>记录时间</th><th>操作</th></tr>
        </thead>
        <tbody>
          <tr v-for="r in records" :key="r.id">
            <td>{{ r.name }}</td><td>{{ r.systolic }}</td><td>{{ r.diastolic }}</td>
